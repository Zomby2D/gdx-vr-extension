...
/** Responsible for initializing the VR system, managing rendering surfaces, getting tracking device poses, submitting the
 * rendering results to the HMD and rendering the surfaces side by side to the companion window on the desktop. Wrapper around
 * OpenVR.
 *
 * FIXME add multisampling plus draw/resolve buffers */
public class OpenVRContext implements Disposable {
	...
	/** Space in which matrices and vectors are returned in by {@link VRDevice} methods taking a {@link Space}. In case
	 * {@link Space#World} is specified, all values are transformed by the {@link Matrix4} set via
	 * {@link OpenVRContext#setTrackerSpaceOriginToWorldSpaceTransform(Matrix4)}. */
	...
	public static enum VRDeviceProperty {
		...
		TrackingSystemName_String(1000), ModelNumber_String(1001), SerialNumber_String(1002), RenderModelName_String(
			1003), WillDriftInYaw_Bool(1004), ManufacturerName_String(1005), TrackingFirmwareVersion_String(
				1006), HardwareRevision_String(1007), AllWirelessDongleDescriptions_String(1008), ConnectedWirelessDongle_String(
					1009), DeviceIsWireless_Bool(1010), DeviceIsCharging_Bool(1011), DeviceBatteryPercentage_Float(1012), // 0 is
																																							// empty), 1
																																							// is full
		// FIXME
		// StatusDisplayTransform_Matrix34 (1013),
		Firmware_UpdateAvailable_Bool(1014), Firmware_ManualUpdate_Bool(1015), Firmware_ManualUpdateURL_String(
			1016), HardwareRevision_Uint64(1017), FirmwareVersion_Uint64(1018), FPGAVersion_Uint64(1019), VRCVersion_Uint64(
				1020), RadioVersion_Uint64(1021), DongleVersion_Uint64(1022), BlockServerShutdown_Bool(
					1023), CanUnifyCoordinateSystemWithHmd_Bool(1024), ContainsProximitySensor_Bool(
						1025), DeviceProvidesBatteryStatus_Bool(1026), DeviceCanPowerOff_Bool(1027), Firmware_ProgrammingTarget_String(
							1028), DeviceClass_Int32(1029), HasCamera_Bool(1030), DriverVersion_String(
								1031), Firmware_ForceUpdateRequired_Bool(1032), ViveSystemButtonFixRequired_Bool(1033),

		// Properties that are unique to TrackedDeviceClass_HMD
		ReportsTimeSinceVSync_Bool(2000), SecondsFromVsyncToPhotons_Float(2001), DisplayFrequency_Float(2002), UserIpdMeters_Float(
			2003), CurrentUniverseId_Uint64(2004), PreviousUniverseId_Uint64(2005), DisplayFirmwareVersion_Uint64(
				2006), IsOnDesktop_Bool(2007), DisplayMCType_Int32(2008), DisplayMCOffset_Float(2009), DisplayMCScale_Float(
					2010), EdidVendorID_Int32(2011), DisplayMCImageLeft_String(
						2012), DisplayMCImageRight_String(2013), DisplayGCBlackClamp_Float(2014), EdidProductID_Int32(2015),
		// FIXME
		// CameraToHeadTransform_Matrix34 (2016),
		DisplayGCType_Int32(2017), DisplayGCOffset_Float(2018), DisplayGCScale_Float(2019), DisplayGCPrescale_Float(
			2020), DisplayGCImage_String(2021), LensCenterLeftU_Float(2022), LensCenterLeftV_Float(2023), LensCenterRightU_Float(
				2024), LensCenterRightV_Float(2025), UserHeadToEyeDepthMeters_Float(2026), CameraFirmwareVersion_Uint64(
					2027), CameraFirmwareDescription_String(2028), DisplayFPGAVersion_Uint64(2029), DisplayBootloaderVersion_Uint64(
						2030), DisplayHardwareVersion_Uint64(2031), AudioFirmwareVersion_Uint64(2032), CameraCompatibilityMode_Int32(
							2033), ScreenshotHorizontalFieldOfViewDegrees_Float(2034), ScreenshotVerticalFieldOfViewDegrees_Float(
								2035), DisplaySuppressed_Bool(2036), DisplayAllowNightMode_Bool(2037),

		// Properties that are unique to TrackedDeviceClass_Controller
		AttachedDeviceId_String(3000), SupportedButtons_Uint64(3001), Axis0Type_Int32(3002), // Return value is of type
		...
		FieldOfViewLeftDegrees_Float(4000), FieldOfViewRightDegrees_Float(4001), FieldOfViewTopDegrees_Float(
			4002), FieldOfViewBottomDegrees_Float(
				4003), TrackingRangeMinimumMeters_Float(4004), TrackingRangeMaximumMeters_Float(4005), ModeLabel_String(4006),
		...
		NamedIconPathDeviceSearching_String(5002), // PNG for static icon), or GIF for animation), 50x32 for headsets and 32x32 for
																	// others
		NamedIconPathDeviceSearchingAlert_String(5003), // PNG for static icon), or GIF for animation), 50x32 for headsets and 32x32
																		// for others
		NamedIconPathDeviceReady_String(5004), // PNG for static icon), or GIF for animation), 50x32 for headsets and 32x32 for
															// others
		NamedIconPathDeviceReadyAlert_String(5005), // PNG for static icon), or GIF for animation), 50x32 for headsets and 32x32 for
																	// others
		NamedIconPathDeviceNotReady_String(5006), // PNG for static icon), or GIF for animation), 50x32 for headsets and 32x32 for
																// others
		NamedIconPathDeviceStandby_String(5007), // PNG for static icon), or GIF for animation), 50x32 for headsets and 32x32 for
																// others
		NamedIconPathDeviceAlertLow_String(5008), // PNG for static icon), or GIF for animation), 50x32 for headsets and 32x32 for
																// others

		// Vendors are free to expose private debug data in this reserved region
		VendorSpecific_Reserved_Start(10000), VendorSpecific_Reserved_End(10999);
	...
	/** Creates a new OpenVRContext, initializes the VR system, and sets up rendering surfaces with depth attachments. */
	public OpenVRContext () {
		this(1, false);
	}

	/** Creates a new OpenVRContext, initializes the VR system, and sets up rendering surfaces.
	 *
	...
	public OpenVRContext (float renderTargetMultiplier, boolean hasStencil) {
	...
	private void setupEye (Eye eye, int width, int height, boolean hasStencil) {
		...
		OpenVRCamera camera = new OpenVRCamera(this, eye);
		camera.near = 0.1f;
		camera.far = 1000f;
	...
	/** Returns the tracker space to world space translation offset. All positional vectors returned by {@link VRDevice} methods
	 * taking a {@link Space#World} are multiplied offset by this vector. This allows offsetting {@link VRDevice} positions and
	...
	/** Returns the tracker space to world space rotation offset. All rotational vectors returned by {@link VRDevice} methods
	 * taking a {@link Space#World} are rotated by this offset. This allows offsetting {@link VRDevice} orientations in world
	 * space. The matrix needs to only have rotational components. */
	...
	public void pollEvents () {
		...
		for (int device = 0; device < VR.k_unMaxTrackedDeviceCount; device++) {
			...
			pose.angularVelocity.set(trackedPose.vAngularVelocity().v(0), trackedPose.vAngularVelocity().v(1),
			...
			if (devices[device] != null) {
				...
				if (devices[device].modelInstance != null) {
					devices[device].modelInstance.transform.idt().translate(trackerSpaceOriginToWorldSpaceTranslationOffset)
						.mul(trackerSpaceToWorldspaceRotationOffset).mul(pose.transform);
				}
			}
		}
	...
	/** Start rendering to the rendering surface for the given eye. Complete by calling {@link #endEye()}. */
	...
	/** Completes rendering and submits the rendering surfaces to the head mounted display. */
	public void end () {
		if (!renderingStarted) throw new GdxRuntimeException("Call begin() before end()");
		renderingStarted = false;

		VRCompositor.VRCompositor_Submit(VR.EVREye_Eye_Left, perEyeData[Eye.Left.index].texture, null,
			VR.EVRSubmitFlags_Submit_Default);
		VRCompositor.VRCompositor_Submit(VR.EVREye_Eye_Right, perEyeData[Eye.Right.index].texture, null,
			VR.EVRSubmitFlags_Submit_Default);
	}
	...
	/** Resizes the companion window so the rendering buffers can be displayed without stretching. */
	...
	/** Renders the content of the given eye's rendering surface to the entirety of the companion window. */
	...
	private Model loadRenderModel (String name) {
		...
		RenderModelTextureMap renderModelTexture = new RenderModelTextureMap(
			texturePointer.getByteBuffer(RenderModelTextureMap.SIZEOF));

		// convert to a Model
		Mesh mesh = new Mesh(true, renderModel.unVertexCount(), renderModel.unTriangleCount() * 3, VertexAttribute.Position(),
	...
	/** Keeps track of per eye data such as rendering surface, or {@link OpenVRCamera}. */
	public static class VRPerEyeData {
		...
		/** the {@link OpenVRCamera} for this eye **/
		public final OpenVRCamera camera;
		/** used internally to submit the frame buffer to OpenVR **/
		final org.lwjgl.openvr.Texture texture;

		VRPerEyeData (FrameBuffer buffer, TextureRegion region, OpenVRCamera cameras) {
			this.buffer = buffer;
			this.region = region;
			this.camera = cameras;
			this.texture = org.lwjgl.openvr.Texture.create();
			this.texture.set(buffer.getColorBufferTexture().getTextureObjectHandle(), VR.ETextureType_TextureType_OpenGL,
	...
	/** Represents the pose of a {@link VRDevice}, including its transform, velocity and angular velocity. Also indicates whether
	 * the pose is valid and whether the device is connected. */
	...
	public class VRDevice {
		 ...
		 *
		 * <p>
		 * <strong>Note</strong>: the role is not reliable! If one controller is connected on startup, it will have a role of
		 * {@link VRControllerRole#Unknown} and retain that role even if a second controller is connected (which will also haven an
		 * unknown role). The role is only reliable if two controllers are connected already, and none of the controllers
		 * disconnects during the application life-time.</br>
		 * At least on the HTC Vive, the first connected controller is always the right hand and the second connected controller is
		 * the left hand. The order stays the same even if controllers disconnect/reconnect during the application life-time.
		 * </p>
		*/
		...
		/** Trigger a haptic pulse (vibrate) for the duration in microseconds. Subsequent calls to this method within 5ms will be
		 * ignored.
		...
		/** @return a {@link ModelInstance} with the transform updated to the latest tracked position and orientation in world space
		...
		public String toString () {
			return "VRDevice[manufacturer=" + getStringProperty(VRDeviceProperty.ManufacturerName_String) + ", model="
				+ getStringProperty(VRDeviceProperty.ModelNumber_String) + ", renderModel="
				+ getStringProperty(VRDeviceProperty.RenderModelName_String) + ", index=" + pose.index + ", type=" + type + ", role="
				+ role + "]";
		}
	}
